...
task Build -depends Clean {
  ...
  Copy-Item -Path $docDir\readme.txt -Destination $workingDir\
  ...
  $xml = [xml](Get-Content "$workingSourceDir\phrogGLControl\phrogGLControl.csproj")
...
task Package -depends Build {
  foreach ($build in $script:enabledBuilds)
  {
    ...
    $sourcePath = "$workingSourceDir\phrogGLControl\bin\Release\$finalDir"
  ...
  if ($buildNuGet)
  {
    ...
    exec { & $script:msBuildPath "/t:pack" "/p:IncludeSource=true" "/p:Configuration=Release" "/p:TargetFrameworks=`"$targetFrameworks`"" "$workingSourceDir\phrogGLControl\phrogGLControl.csproj" }

    mkdir $workingDir\NuGet
    move -Path $workingSourceDir\phrogGLControl\bin\Release\*.nupkg -Destination $workingDir\NuGet
  }
  ...
  Copy-Item -Path $workingDir\LICENSE.md -Destination $workingDir\Package\Source\
...
{
  $projectPath = "$workingSourceDir\phrogGLControl.sln"
